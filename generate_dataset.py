...
def saveImage(image, userName, imgId):
    ...
    cv2.imwrite("dataset/{}/{}_{}.jpg".format(userName, userName.lower(), imgId), image)
...
while True:
    ...
    if key == ord('s'):
        # If count is less than 8 then save the image
        if count <= 8:
            #roi_img = originalImg[coords[1] : coords[1] + coords[3], coords[0] : coords[0] + coords[2]]
            #saveImage(roi_img, userName, count)
            saveImage(originalImg, userName, count)
            count += 1
        else:
            break
